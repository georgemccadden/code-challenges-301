...
const forLoopTwoToThe = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    watchMeLoop.push(Math.pow(2,input[i]));
  }
  return watchMeLoop;
};
...
const forEachTwoToThe = (input) => {
  ...
  input.forEach(element => {
    noForLoops.push(Math.pow(2, element));
  });
  return noForLoops;
};
...
const mapTwoToThe = (input) => {
  // Solution code here...
  const usingMap = input.map(element => Math.pow(2, element));
  return usingMap;
};
...
const charCode = (input) => {
  // Solution code here...
  const charCodeResults = input.map(element => element.charCodeAt());
  return charCodeResults;
};
...
const extractAbilities = (abilities) => {
  // Solution code here...
  const justTheNames = abilities.map(object => {
