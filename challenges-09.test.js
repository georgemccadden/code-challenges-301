...
const countNumberOfElements = (input) => {
  // Solution code here...
  const anotherWayToCount = accumulator => accumulator + 1;
  
  return input.reduce(anotherWayToCount, 0);
};
...
const calculateAverage = (input) => {
  // Solution code here...
  return input.reduce((accumulator, value) => accumulator + value) / input.length;
};
...
const reversedString = (input) => {
  // Solution code here...
  return input.split('').reduce((accumulator, value) => value + accumulator);
};
